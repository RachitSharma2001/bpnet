...
# Added Class for applying batchnorming
class BatchNormDataset(Dataset):
    ...
    def normalize_column(self, np_row, col):
        row = np_row[:,:,col]
        #print("In normalize column, here is np_row[:,:,col] -> ", np_row[:,:,col])
    ...
    def normalize(self, data):
        ...
        #print("Np_row specifically: ", np_row)

        # go through columns to batchnorm, and normalize by each col
        for i in range(7):
            data['seq'][:,:,i] = self.normalize_column(np_row, i)
            '''print("Column after batch norm: ", data['seq'][:,:,i+4])
            print("Column we will batch norm: ", i+4)
            print("Column before batch norm: ", data['seq'][:,:,i+4])'''
    ...
    def batch_train_iter(self, cycle=True, **kwargs):
        ...
        Args:
          ...
          **kwargs: Arguments passed to self.batch_iter(**kwargs)
        """
        print("In batch_train_iter, cycle is: ", cycle)

        '''obj = ((to_numpy(x["inputs"]), to_numpy(x["targets"]))
                for x in iterable_cycle(self._batch_iterable(**kwargs)))
        ...
        if cycle:
            return ((self.normalize(self.to_numpy(x["inputs"])), self.to_numpy(x["targets"]))
                    for x in iterable_cycle(self._batch_iterable(**kwargs)))
            '''return ((self.to_numpy(x["inputs"]), self.to_numpy(x["targets"]))
                    for x in iterable_cycle(self._batch_iterable(**kwargs)))'''
        else:
            return ((self.normalize(x["inputs"]), x["targets"]) for x in self.batch_iter(**kwargs))
            '''return ((x["inputs"], x["targets"]) for x in self.batch_iter(**kwargs))'''

    def __init__(self, np_dataset, attrs=None):
        """
        Args:
          data: any arbitrarily nested dict/list of np.arrays
            with the same first axis size
